...
class SettingsSeeder extends Seeder
{
    ...
    public function run(): void
    {
        $settings = [
            ...
            // --- FAQ мини-аппа (редактируется на странице «FAQ (мини-апп)») ---
            ['faq_items', json_encode([
                ['q' => 'Что делает Радистка Cat?', 'a' => 'Прозванивает вашу базу номеров и показывает, какие абоненты живые и доступны, а какие — нет. Платите только за результат по живым контактам.'],
                ['q' => 'Какие форматы файлов принимаются?', 'a' => 'Таблицы .xlsx и текстовые .txt. Номера распознаются в любом формате (+7, 8, без кода) и автоматически нормализуются и дедуплицируются.'],
                ['q' => 'Сколько стоит проверка?', 'a' => 'Цена зависит от ГЕО и считается за 1000 номеров. Точную сумму показывает калькулятор и расчёт перед запуском проверки. Первые номера — бесплатно.'],
                ['q' => 'Как пополнить баланс?', 'a' => 'На вкладке «Пополнить» укажите сумму и оплатите через менеджера — баланс зачислится автоматически после подтверждения.'],
                ['q' => 'Когда придёт результат?', 'a' => 'В рабочие часы обзвон стартует сразу, результат приходит отдельным файлом. Вне рабочих часов проверка запланируется на ближайшее утро по часовому поясу абонента.'],
                ['q' => 'Как работает реферальная программа?', 'a' => 'Делитесь своей ссылкой и получаете процент с каждого пополнения приглашённых. Реферальный баланс можно вывести по достижении условий.'],
            ], JSON_UNESCAPED_UNICODE), 'json', 'content', 'FAQ мини-аппа [{q,a}]'],
        ];
